...
export async function meusEmprestimos(token) {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8080";

  const res = await fetch(`${apiUrl}/biblioteca/meus-emprestados`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${token}`,
    },
...
export async function deleteUser(cpf, token) {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8080";
    const res = await fetch(`${apiUrl}/biblioteca/usuarios/${cpf}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
...
export async function loadUsers(token) {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8080";
    const res = await fetch(`${apiUrl}/biblioteca/usuarios`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
